...
# Arquivos individuais a comparar
declare -A FILES=(
...
# Arquivo temporário para armazenar lista de diferenças
DIFF_LIST=$(mktemp)

# Função para comparar dois diretórios
comparar_dirs() {
    local DIR1="$1"
    local DIR2="$2"
...
# Função para comparar arquivos individuais
comparar_arquivos() {
...
if [ -s "$DIFF_LIST" ]; then
    cut -d'|' -f2 "$DIFF_LIST"  # Mostrar caminhos do Dotfiles
else
...
# Segunda passada: mostrar diff interativo
